...
def _normalize_shape_arg(value, ndim):
    if isinstance(value, int):
        return [value] * ndim
    return list(value)


def random_patch_start(volume_shape, patch_size):
    """Sample a valid patch start index for a 2D/3D volume."""
    patch_size = _normalize_shape_arg(patch_size, len(volume_shape))
    starts = []
    for dim, size in zip(volume_shape, patch_size):
        if dim <= size:
            starts.append(0)
        else:
            starts.append(np.random.randint(0, dim - size + 1))
    return tuple(starts)


def iter_patch_starts(volume_shape, patch_size, stride):
    """Generate deterministic sliding-window patch starts."""
    patch_size = _normalize_shape_arg(patch_size, len(volume_shape))
    stride = _normalize_shape_arg(stride, len(volume_shape))

    ranges = []
    for dim, size, step in zip(volume_shape, patch_size, stride):
        if dim <= size:
            ranges.append([0])
            continue
    ...
    for start in np.ndindex(*[len(r) for r in ranges]):
        yield tuple(ranges[axis][idx] for axis, idx in enumerate(start))


def extract_patch(image, start, patch_size, pad_value=None):
    """
    Extract a patch from a 2D/3D image.
    If the patch extends beyond the image boundary, it is padded to target size.
    """
    patch_size = _normalize_shape_arg(patch_size, image.ndim)
    if pad_value is None:
        pad_value = np.min(image)

    slices = []
    pad_width = []
    for axis, (axis_start, axis_size, dim) in enumerate(zip(start, patch_size, image.shape)):
        axis_end = axis_start + axis_size
        src_start = max(axis_start, 0)
        src_end = min(axis_end, dim)
        slices.append(slice(src_start, src_end))

        pad_before = max(0, -axis_start)
        pad_after = max(0, axis_end - dim)
